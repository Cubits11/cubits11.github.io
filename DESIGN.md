...
- **Options.** (a) An essay arguing the gap exists. (b) A product-style
  landing page for a "standard". (c) A census: literal inclusion wording
  locked in repository history before row classification, every row bound to
  its primary source, counts recomputed from a
...
- **Choice.** (c). The headline ("among N, M shared-basis, K heterogeneous
  joint-evidence artifacts") is computed, never typed; while no row was examined the page said "no
