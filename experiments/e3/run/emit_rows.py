...
experiments/e3/freeze/LICENSE-OUTPUTS.md forbids redistributing OR-Bench text.
"""
...
def main() -> int:
    cfg = json.loads((E3 / "e3_config.json").read_text())
        ...
        for setname, stratum in SETS.items():
            ...
            for slot, per_item in probs.items():
                ...
                for item_id, p in per_item.items():
                    fh.write(json.dumps({
                        "experiment": "E3", "freeze": cfg["frozen"], "seed": cfg["seed"],
