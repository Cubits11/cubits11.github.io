#!/usr/bin/env python3
"""E8 runner: every claim-critical choice comes from a validated contract.
...
What the runner takes and gives:
    ...
    python3 experiments/e8/run/runner.py --contract CONTRACT.json --scores SCORES.json --out DIR
...
are edges U and V of the plane and remain open. E8 has no frozen contract
today, so this runner has produced no observation row; tests/test_e8_runner.py
exercises it on synthetic scores under a constructed contract and shows that a
changed contract changes what it selects. Frozen experiments are not replayed
through it: reuse of frozen rows is not reuse of confirmatory status.
"""
...
def execute(contract: dict, scores: dict) -> tuple[list[dict], dict]:
    ...
    for item_id in sorted(evaluation):
        ...
        rows.append({"experiment": "E8", "contract_id": contract["id"], "pool": contract["execution_plan"]["evaluation"],
    ...
    result = {"experiment": "E8", "contract_id": contract["id"], "estimand": contract["estimand"],
...
def run(contract_path: Path, scores_path: Path, out: Path, require_frozen: bool = True) -> dict:
    ...
    result["provenance"] = {"contract": {"path": str(contract_path), "sha256": sha256(contract_path)},
                            ...
                            "runner": {"path": "experiments/e8/run/runner.py", "sha256": sha256(Path(__file__))},
