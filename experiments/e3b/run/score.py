#!/usr/bin/env python3
"""E3 scoring. Refuses to score harmful items until e3b_config.json is committed.

Guards emit a probability for the "unsafe/injection" class. A flag is
prob >= threshold. Thresholds come from e3b_config.json and never from here.
"""
...
FREEZE = ROOT / "experiments/e3b/freeze"
CONFIG = ROOT / "experiments/e3b/e3b_config.json"
GUARDS = {
...
def texts(name: str) -> list[tuple[str, str]]:
    pool = "gandalf.csv" if name == "items_harmful" else "or-bench-80k.csv"
...
def main() -> int:
    ...
    if a.set == "items_harmful" and not committed(CONFIG):
        print("REFUSED: e3b_config.json is not committed. Calibrate and commit the "
